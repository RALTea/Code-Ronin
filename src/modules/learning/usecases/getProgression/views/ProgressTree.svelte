...
{#snippet bullet(task: TaskTreeItem, index: number)}
	<li class="" bind:this={treeElements[index]}>
		<a class="flex items-center gap-2 relative cursor-pointer" href={task.id}>
			<div
			...
			{#if task.nextTaskId}
				{@render path(task.isCompleted, index)}
			{/if}
		</a>
	</li>
{/snippet}

{#snippet path(isCompleted: boolean, index: number)}
	<div
